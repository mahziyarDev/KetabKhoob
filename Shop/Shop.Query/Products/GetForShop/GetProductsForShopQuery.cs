...
internal class GetProductsForShopQueryHandler : IBaseQueryHandler<GetProductsForShopQuery, ProductShopResult>
{
    ...
    public async Task<ProductShopResult> Handle(GetProductsForShopQuery request, CancellationToken cancellationToken)
    {
        ...
        var count = await sqlConnection.QueryFirstAsync<int>(sql, new { status = SellerStatus.Accept });
        var result = await sqlConnection.QueryAsync<ProductShopDto>(resultSql,
            new { skip, take = @params.Take, status = SellerStatus.Accept });
